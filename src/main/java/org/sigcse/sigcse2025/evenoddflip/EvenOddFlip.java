 ...
 * It should provide a single constructor accepting an `int`, which sets the initial state of the instance.
 *
 * Provide two instance methods.
 * 1. `setValue` accepts a single `int` and updates the stored value.
 * You should `assert` that the value passed to `setValue` is even if the current value is odd, and odd if the current
 * value is even.
